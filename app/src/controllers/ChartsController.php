...
final class ChartsController extends BaseController
{
    ...
    public function getUdooRealtimeList($usn, $sensorId)
    {
        /** usn과 sensor_id로 1분내의 데이터를 가져옴
         **
         */
        $sql = "select * 
        from sensor natural join aqi_data 
    ...
    public function UdooRealtimeChart(Request $request, Response $response, $args)
    {
        /** sensorId가 0일 경우 모든 sensorId 선택한 것, 음수일 경우 잘못된 값 입력
         ** 이외에는 각 값이 sensor_id 
         ** 정상일 경우 result => 0, data => air data 값 전부, 
        ...
        if ($sensorId == 0) {
            ...
            $resultExec = $this->getUdooRealtimeList($usn, $sensorId);
